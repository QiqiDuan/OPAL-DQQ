...
/**
 * HOST: Handle the CUDA Errors for GPU.
 */
#define HCE( cuda_expr ) { assertGpuError( ( cuda_expr ), __FILE__, __LINE__ ); }
inline void assertGpuError( const cudaError_t error_index, 
 ...
 */
__global__ void devSetRngState( const unsigned total_num_threads, 
        const unsigned rng_seed, curandState *dev_rgn_state ) {
 ...
 */
__global__ void devGenUniformRand( const unsigned total_num_threads, 
...
/**
 * HOST: Calculate the Average Value for the One-Dimensional Numerical Array.
 */
...
int main( void ) {
    ...
    float *rand_samples;
    for ( unsigned ind_num_threads = 1; ind_num_threads <= 1000000; ind_num_threads *= 10 ) {
        printf( "ind_num_threads = %u ---> ", ind_num_threads );
    ...
    for ( unsigned ind_iter = 1; ind_iter <= num_iter; ind_iter++ ) {
        ...
        if ( ind_iter == 1 || ind_iter % 500 == 0 || ind_iter == num_iter ) {
            printf( "ind_iter = %u ---> rand_samples[%i] = %5.2lf, [%u] = %5.2lf, [%u] = %5.2lf\n",
            ind_iter,
            0, rand_samples2[ 0 ],
    ...
    printf("avg_fst = %5.2lf && avg_half = %5.2lf && avg_end = %5.2lf\n", 
